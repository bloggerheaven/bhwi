...
class Bhwi {
  constructor(id:number, options:any) {
    new Bhwi.BhwiInitializer(id, options);
  }
}
    ...
    _validateOptions() {
      if (this.options.type == 'instagram') {
        if (this.options.client_id == null) {
          throw new Error('For the instagram API the "client_id" is needed.')
        }
      } else {
        if (this.options.url == null) {
          throw new Error('For the bhwi API the "url" is needed.')
        }
      }
    }
  }
...
// not cross browser supported, see: http://caniuse.com/#feat=touch
  class BhwiTouch {
    ...
    initEvents() {
      ...
      doc.on('touchstart', this.dom_element, (event:Event) => {
        ...
        this.bhwi_helper.delay('touchstart', () => {
          this._start()
        }, 200)
      });
      doc.on('touchend', this.dom_element, (event:Event) => {
        this.endEvent = event;
        this.bhwi_helper.delay('touchend', () => {
          this._end()
        }, 200)
      });
    }
  ...
  class BhwiSlider {
    ...
    _setImage(bhwi_image:BhwiImage) {
      if (this.current_dom != null) {
        this.current_dom.fadeOut(this.speed / 2);
      }
  ...
  export class BhwiInitializer {
    ...
    _callApi() {
      jQuery.getJSON(this.bhwi_user.url).done((insta_posts) => {
        jQuery.each(this.bhwi_user._extractData(insta_posts), (index, insta_posts) => {
          ...
          if (this.bhwi_options.options.preloading_images) {
            this.bhwi_images.preloadingImages();
          }
        });
        this._decideInit();
      });
    ...
    _basicVaildation(id:any, options:any) {
      ...
      if (options == null) {
        throw new Error('The param "options" are required.')
      }
    }
  }
}
